<template>
  <div class="page page--room">
    <section class="section section--room section--mmt p-0">
          ...
          <Sidebar name="userlist" ref="userList">
            <template v-slot:header>
              <div class="userlist__actions">
                ...
                <span class="section_title">Online Users</span>
                <ion-icon @click="toggleUserList" name="backspace" class="icon"></ion-icon>
              </div>
            </template>
            <template v-slot:body>
              <input type="text" class="rooms__search-input" placeholder="Search user by name"
                     v-model.trim="searchInput"/>
              <ul class="chat__userlist" v-if="getCurrentRoom && filteredUsers">
                  ...
                  <li class="chat__user" v-for="user in filteredUsers" :key="user.lookup._id">
                    <div class="chat__user-item">
                      <div class="chat__user-image">
                        <img v-if="user.lookup.social.id === null" :src="user.lookup.image" class="chat__user-avatar"
                             alt/>
          ...
          <div class="chat__content" v-bind:class="{ mlzero: !sidebarVisible }">
            <div class="chat__header" v-if="room">
              ...
              <div class="chat__actions">
                <ion-icon name="return-up-back" @click="leaveRoom" class="icon"></ion-icon>
      ...
      <Modal name="editRoom" ref="editRoom" v-if="getCurrentRoom">
        <template v-slot:header>
          <h2 class="text-upper">Edit Room: {{ getCurrentRoom.name }}</h2>
        </template>
        <template v-slot:body>
      ...
      <Modal name="roomDetails" ref="roomDetails" v-if="getCurrentRoom && messages">
        <template v-slot:header>
          <h2 class="lead text-upper">Room Details: {{ getCurrentRoom.name }}</h2>
        </template>
        <template v-slot:body>
          <div class="infobox">
            ...
            <div class="infobox__item">
              ...
              <span class="infobox__item--right">
                                {{ getCurrentRoom.user ? getCurrentRoom.user.handle : 'Unknown User' }}
                            </span>
            </div>
            <div class="infobox__item">
  ...
  created() {
    axios
        ...
        .then((res) => {
          ...
          if (!res.data.access) {
            if (!res.data.accessIds.includes(this.getUserData._id) && this.getUserData._id !== res.data.user._id) {
