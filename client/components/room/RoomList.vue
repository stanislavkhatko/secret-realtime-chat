        ...
        <div class="rooms" v-if="rooms">
          ...
          <transition name="slideDown">
            <ul class="rooms__list">
              ...
              <li v-else class="rooms__list-item">No Chats</li>
            </ul>
          </transition>
          <!-- Private Room Enter Modal -->
          <Modal name="private-room" ref="privateRoom">
            <template v-slot:header>
              <h2 class="text-upper">Enter {{ privateRoomName || 'Private Room' }}</h2>
            </template>
            <template v-slot:body>
              <form @submit="handlePrivateRoomCheck" class="form form--nbs pt-3">
          ...
          <Modal name="create-room" ref="createRoom">
            ...
            <template v-slot:body>
              <form @submit="handleCreateRoom" class="form form--nbs pt-3">
                <div class="form__input-group">
                  <ion-icon name="list" class="form__icon"></ion-icon>
                  <input
                      type="text"
                      name="room_name"
                      class="form__control"
          ...
          <div class="rooms__actions">
            <a @click="openModal" class="btn btn--info">Create Chat</a>
            <a @click="fetchRoomData" class="btn btn--info">Refresh Chats</a>
          </div>
        </div>
      </div>
    </section>
...
export default {
  ...
  methods: {
    ...
    handleRoomClick(room) {
      if (room.access || this.getUserData._id === (room.user && room.user._id) || room.accessIds.includes(this.getUserData._id)) {
        this.enterRoom(room);
      } else {
