 ...
 */
router.post('/', [passport.authenticate('jwt', {session: false}), checkCreateRoomFields],
    async (req, res) => {
        let errors = [];
        ...
        if (room) {
            if (room.name === req.body.room_name) {
                errors.push({param: 'room_taken', msg: 'Room name already taken'});
            }
            ...
            newRoom.save().then(async room => {
                return res.status(200).json(await Room.populate(room, {path: 'user', select: 'username'}));
            }).catch(err => {
                return res.json(err);
            });
        }
